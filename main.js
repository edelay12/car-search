...
function getResultsRetry(params, n) {
  ...
  fetch(url)
    ...
    .catch(err => {
      ...
      $("#js-error").text(
        "Server Error: Please try your search again in a few seconds"
      );
...
function displayResults(ResponseJson) {
  ...
  $(".numberResults").html(
    `Search Results: <mark class="number">${
      ResponseJson.num_found < 15 ? "" : start + 15 + " of "
    }(${ResponseJson.num_found})</mark>`
  );
...
function watchFilters(cordinates) {
  $(".filter_form").on("submit", function(e) {
    ...
    let newParams = {
      ...
      distance:
        currentSearch.dealerType == 2
          ? "&radius=" + $("#range_disp").val()
          : "",
      priceRange: getPrice(),
      milesRange: getMiles(),
...
function updateSearchBar() {
  let userSearch = {
    ...
    userBuild:
      currentSearch.userYear.val() +
      " " +
      currentSearch.userMake.val() +
      " " +
      currentSearch.userModel.val(),
...
function newSearch() {
  $(".newSearch, .labels, .searchIconContainer").on("click", function() {
    $(".newSearchDealerTypeContainer").show();
